...
# ************************** PyMKS ****************************
def pymks_elasticity(volumefraction, radius, T_service, n_samples):
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    E_eff = np.array(n_samples, dtype=float)
    ...
    if (n_samples > 1):
        ...
        E_eff = stress_pred.mean()/strain_pred.mean()
...
if __name__ == '__main__':
    ...
    #-------------------------------------------------------------- Knime starts
    Elasticity = np.array([0] * n_samples, dtype=float)

    print "PyMKS:", time.asctime(time.localtime(time.time()))
    Elasticity = pymks_elasticity(vf, ra)
    Elasticity *= 1000
    ...
    dictMKS.append(('optimumRadius', ({'value': ra},
                   {'unit': 'meter'})))
    dictMKS.append(('youngsModulus', ({'value': Elasticity[ind_xml]},
                   {'unit': 'MPa'})))
