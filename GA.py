...
#----------------------------------------------------------------------------------------------
	...
	if (test_model == 0):
		...
		microstructure = EQ_TCAPI(gen,no_samples,start_samples,len(variables[0]),variables)
		microstructure = np.fromiter(microstructure, dtype=float, count=no_samples*5).reshape(no_samples,5)
...
#		Ttest=variables[:,3]
		Ttest= [1123. for x in xrange(no_samples)]
...
# *** Physical Inputs ***
	no_samples = 4
	no_variables = 3
	L_sample_range = [ 0.219, 0.186, 1445.]
	H_sample_range = [ 0.219, 0.186, 1445.]

# 1983                    Ni-0.14Al           : 550C
# 1991                    Ni-0.12Al           : 500C
# 2008_Effects_sudbrack:  Ni-0.098Al-0.083Cr  : 800C
# 2010_on_Booth:          Ni-0.065Al-0.095Cr  : 600C 
# 2008_Effects_Booth:     Ni-0.075Al-0.085Cr  : 600C
# 2006_Temporal_Sudbrack: Ni-0.052Al-0.142Cr  : 600C

	T_service = 1123

# +++ Numerical Parameters +++
	generations = 4
	Memory = [6 for x in xrange(no_variables)]
	History = []
	keep_history = 1
	History_Check = 0
...
#		Evolution = open('avg_fitness.dat', 'w')
...
# +++++++ Loading TC API function +++++++
	...
	EQ_TCAPI.argtypes = (ctypes.c_int,ctypes.c_int,ctypes.c_int,ctypes.c_int,DoubleArrayType)
