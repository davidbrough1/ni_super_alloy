...
double *API_TC(int generation,int no_samples, int start_samples, int no_variables, double *XT)
{
 ...
 /* ++++ Variables for process ++++ */ 
  ...
  double *results;
  const int no_results=no_samples*5;
  int count_results;
...
//  const int max_time_step=2;
  ...
  const double no_ini_nu_site=4E+26;   //2E+26;   //2008B: model2_5E+26/model3_2.5E+26
  double no_nu_site;
  double nu_site;
  ...
  double diffusivity[TC_MAX_NR_OF_ELEMENTS], diffusivity_off[TC_MAX_NR_OF_ELEMENTS];
  double diff;
...
/* Qing et al., Analytical treatment..., Acta, 2008 */
  ...
  double tot_precipitate[max_time_step+1], precipitates[4][max_time_step+1];
  double swap;
...
// Unit: MPa per (at fraction)^(-0.5)
  ...
  int testtt0,testtt1,testtt2,testtt3;

  FILE *file_id=fopen("C_samples.dat","a");
  FILE *file_ys=fopen("YStress.dat","a");
  FILE *file_ki=fopen("kinetics.dat","a");
  FILE *file_comp=fopen("composition.dat","a");
  FILE *file_EAPB=fopen("EAPB.dat","a");

  FILE *file_ES=fopen("ES_Kinetics.dat","a");
...
// ------ decide the phase index of order phase, FCC_L12 (and?) -----------
      if( (error_diff_Ni > ST_error) && (error_diff_Al > ST_error) ){
        no_ordered+=1;
...
// ---- Checking points of the microstructure ----
    if(tot_Vf<0.9999999 || VF[ind_L12]>0.99){
      ...
      results[count_results]=0.;
      results[count_results+1]=0.;
      results[count_results+2]=0.;
      results[count_results+3]=0.;
      results[count_results+4]=0.;
...
//      printf("Chemical Potential: %E, %E, %E \n",chem_poten[i],chem_comp_gp[i],chem_composition[i]);
      ...
      diffusivity[i]=tc_get_value(conditions);
...
//    if(E_intfac<0.027 || E_intfac>0.029){
    if(E_intfac>0.029){
      fprintf(stdout,"Low/High Interfacial Energy: %E \n", E_intfac);
...
//      for(j=1;j<no_elements;j++){
        if(model_velocity==0){
          ...
          diff=0.;
          asprintf(&conditions,"%s%s%s%s%s%s%s","DC(FCC_A1,",chem_elements[i],",",chem_elements[j],",",chem_elements[0],")");
          diff=tc_get_value(conditions);

          eq_chemsq_dM+=diff*(chem_composition[j]-chem_comp_gamma[j])/(C_growth*(chem_comp_gp[i]-chem_comp_gamma[i]));
...
//      printf("eq_chemsq: %E %E %E %E \n",eq_chemsq_dM,C_growth,(chem_comp_gp[i]-chem_comp_gamma[i]),mobility[i]);
      ...
      // ---- from TC_presima ----
      beta_trans+=pow((chem_comp_gp[i]-chem_comp_gamma[i]),2)/(chem_comp_gamma[i]*diffusivity[i]);
...
//    printf("Beta: %E, %E, %E, %E, %E \n",pow(gp_radius0,2),pow(gp_LP,4),beta_trans,chem_comp_gp[1],chem_comp_gamma[1]);
    ...
    printf("# of initial-steady nucleation site: %E, %E, %E, %E, %E \n",no_nu_site,no_ini_nu_site,Zeldfactor,beta_trans,exp(-dGM_critical/(KB*temperature)));
    if(no_nu_site<1E5){
...
//printf("time step: %d \n",time_step);
        ...
        if(onnucleation==0 && oncoarsening==0){   // Nucleation
          ...
          dtime=time_incu*0.1;  //310*exp(-10/(time_step-onnustep));
          tot_time+=dtime;
        }
        else{
...
//                YS_p=YS;
		...
		if( (time_step<=wstep || (time_step%(max_time_step/winterval))==0) ){
		  if(time_step>wstep){
                    fprintf(file_ys,"%E, %E, %E, %E, %E, %E, %E, %E \n",(tot_time+time_incu)/60.,tot_Vf*100.,gp_mean_radius*1E+9,tot_Part,weak_stress,strong_stress,Orwan_stress,YS);
                  }
...
//printf("composition %d: %E %E %E %E \n",i,chem_comp_trans[i],chem_comp_gamma[i],chem_composition[i],chem_comp_gp[i]);
           ...
           /* If the molar volume is not obtained from database, chem_comp_trans[i] may be negative!!  */
		    ...
		    conditions=NULL;
		    asprintf(&conditions,"%s%s%s","X(",chem_elements[i],")");
		    tc_set_condition(conditions,chem_comp_trans[i]);
		  }
		  ...
		  if ( tc_error(&ierr,error,sizeof(error)) ) {
		    results[count_results]=-1.0;
		    fprintf(stdout,"error while calculating equilibrium in transition process: %s\n",error);
		    printf("composition: %E, Volume Fraction: %E \n",chem_comp_trans[1],tot_Vf);
		    tc_reset_error();
		  ...
		  for(i=1;i<no_elements;i++){
		    ...
		    diffusivity[i]=tc_get_value(conditions);
                 ...
                 if(oncoarsening==0){
                   ...
                   eq_chemsq_dM=0.;
   K_LSW=4.*chem_comp_gamma[1]*diffusivity[1]*(2*E_intfac*(mvolume_gp/Avogadro))/(27.*(chem_comp_gp[1]-chem_comp_gamma[1])*KB*temperature);
                   printf("K_LSW: %E \n",K_LSW);
                 }
               }
...
//            for(j=1;j<no_elements;j++){
              if(model_velocity==0){
                ...
                eq_chemsq_dM+=diff*(chem_comp_trans[j]-chem_comp_gamma[j])/(C_growth*(chem_comp_gp[i]-chem_comp_gamma[i]));
...
/*
    printf("---- Radius: %E;  Max Stress: %E, %E ---- \n",gp_mean_radius,weak_stress,strong_stress);
    ...
    dislocation_stress=0.25*shear_modulus*burgers*sqrt(1E+13)/1E+6;   // MPa

    SS_stress=sqrt(SS_stress);
    YS=YS_0+SS_stress+HP_stress+sqrt(pow(dislocation_stress,2)+pow(Prec_stress,2));
    printf("YS Component (SS, HP, Precipitation): %f, %f, %f, %f; Total Yield Stress, %f \n",SS_stress,HP_stress,dislocation_stress,Prec_stress,YS);
*/


// %%%% Output of Results %%%%
/*
    for(j=(ind_XT-no_variables);j<ind_XT;j++){
      fprintf(file_id,"%f, ",XT[j]);
      fprintf(file_EAPB,"%E, ",XT[j]);
    }
    fprintf(file_id,"%f, %f, %f, %f, %f, %f \n",VF[ind_L12],gp_mean_radius,Prec_stress,SS_stress,HP_stress,YS);
    fprintf(file_EAPB,"%E, %E, %E, %E \n",chem_comp_gp[1],chem_comp_gp[2],chem_comp_gp[1]+chem_comp_gp[2],EAPB);
*/

/*
    printf("%d, %d: \n",sam,count_results);
    printf("Interfacial Energy: %E, %E, %E \n",E_intfac,fabs(dHM_int),alpha_Eint);
    printf("b stress: %E %E %E %E %E \n",YS_0+SS_stress,Back_stress,HP_stress,dislocation_stress,Prec_stress);
    printf("VF, Density: %f, %f \n",VF[ind_L12],density_all);
    printf("R, Stress: %E, %E, %E\n",gp_mean_radius,YS_0+SS_stress+HP_stress,Prec_stress);
*/

    for(j=(ind_XT-no_variables);j<ind_XT;j++){
      fprintf(file_ES,"%E, ",XT[j]);
    }
    fprintf(file_ES,"%E, %E, %E, ",chem_comp_gp[1],chem_comp_gp[2],VF[ind_L12]);
    fprintf(file_ES,"%E, %E, %E, %E, %E, ",tot_Vf,gp_mean_radius,tot_time,E_intfac,EAPB);
    fprintf(file_ES,"%E, %E, %E, %E, %E \n",YS_0,SS_stress,HP_stress,Back_stress,Prec_stress);
